...
export default function Home() {
  ...
  const { data: venues } = useListVenues({ query: { enabled: isAuthenticated, retry: false } });
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-9 h-9 bg-blue-600 rounded-xl flex items-center justify-center font-bold text-white text-base">R</div>
              <span className="font-bold text-lg text-foreground">RestoCRM</span>
            </div>
                ...
                <>
                  {firstVenue?.telegram && (
                    <a
                      ...
                      className="w-9 h-9 rounded-lg bg-white/5 hover:bg-blue-600/20 border border-border hover:border-blue-500/40 flex items-center justify-center text-muted-foreground hover:text-blue-400 transition-all"
                      title="Telegram"
                    >
                  ...
                  {firstVenue?.facebook && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-9 h-9 rounded-lg bg-white/5 hover:bg-blue-800/20 border border-border hover:border-blue-700/40 flex items-center justify-center text-muted-foreground hover:text-blue-500 transition-all"
                      title="Facebook"
                    >
                      <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
            ...
            <ul className="space-y-3 text-sm text-muted-foreground">
              {firstVenue?.phone ? (
                <li className="flex items-start gap-2.5">
                  ...
                  <a href={`tel:${firstVenue.phone}`} className="hover:text-foreground transition-colors">{firstVenue.phone}</a>
                </li>
              ) : (
                <li className="flex items-start gap-2.5">
                  <svg className="w-4 h-4 text-muted-foreground/40 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  </svg>
                  <span className="italic text-muted-foreground/50">Telefon kiritilmagan</span>
                </li>
              )}
              {firstVenue?.email ? (
                <li className="flex items-start gap-2.5">
                  ...
                  <a href={`mailto:${firstVenue.email}`} className="hover:text-foreground transition-colors">{firstVenue.email}</a>
                </li>
              ) : (
                <li className="flex items-start gap-2.5">
                  <svg className="w-4 h-4 text-muted-foreground/40 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <span className="italic text-muted-foreground/50">Email kiritilmagan</span>
                </li>
              )}
              {firstVenue?.telegram && (
                <li className="flex items-start gap-2.5">
                  <svg className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M11.944 0A12 12 0 1 0 24 12 12.017 12.017 0 0 0 11.944 0Zm4.96 8.284-2.085 9.818c-.157.709-.57.88-1.154.548l-3.18-2.34-1.534 1.476c-.169.17-.312.311-.64.311l.228-3.221 5.879-5.313c.256-.228-.055-.354-.397-.126L8.29 14.5l-3.099-.97c-.674-.21-.686-.674.14-.998l12.09-4.665c.562-.204 1.054.138.483 1.417Z" />
                  </svg>
                  <a href={`https://t.me/${firstVenue.telegram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors">@{firstVenue.telegram.replace("@", "")}</a>
                </li>
              )}
              {firstVenue?.instagram && (
                <li className="flex items-start gap-2.5">
                  <svg className="w-4 h-4 text-pink-400 shrink-0 mt-0.5" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                  </svg>
                  <a href={`https://instagram.com/${firstVenue.instagram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors">@{firstVenue.instagram.replace("@", "")}</a>
                </li>
              )}
              {firstVenue && !firstVenue.phone && !firstVenue.email && !firstVenue.telegram && !firstVenue.instagram && (
                <li className="text-xs text-muted-foreground/50 italic">
                  Owner sozlamalardan aloqa ma'lumotlarini kiriting
                </li>
              )}
            </ul>
          </div>
