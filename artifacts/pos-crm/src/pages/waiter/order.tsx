...
import { ArrowLeft, Search, Plus, Minus, Trash2, Save, X, ShoppingCart, CheckCircle } from "lucide-react";
...
export default function WaiterOrder() {
  ...
  const tableInfo: TableInfo | null = useMemo(() => {
    ...
    for (const room of rooms as any[]) {
      ...
      if (t) return { id: t.id, number: t.number, name: t.name ?? null, roomId: room.id, roomName: room.name, isOccupied: t.isOccupied ?? false, openOrderId: t.openOrderId ?? null };
    }
    return null;
  }, [rooms, tableId]);
